...
<form action="<?=POST_FORM_ACTION_URI?>" method="POST" class="form" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data">
	<div id="bx-soa-order">
		...
		<input type="hidden" name="ORDER_PROP_15" id="soa-property-15">
		<input type="hidden" name="ORDER_PROP_16" id="soa-property-16">
		<?$choose_shop = $APPLICATION->get_cookie('SELECT_SHOP');
		if($choose_shop):
			$rsStoreProduct = \Bitrix\Catalog\StoreProductTable::getList(array(
				'filter' => array('=STORE_ID'=>$choose_shop),
				'limit' => 1,
			...
			if($arStoreProduct=$rsStoreProduct->fetch()){
				/*$explode_name = explode(',',$arStoreProduct['STORE_TITLE']);
				if($explode_name[1]){$arStoreProduct['STORE_TITLE'] = $explode_name[1];}*/
				$choose_shop_title = $arStoreProduct['STORE_TITLE'];
			}
		endif;?>
		<input type="hidden" name="ORDER_PROP_13" value="<?=$choose_shop_title?>" id="soa-property-13">
		...
		<div class="cart-order">
			...
			<div class="cart-order__group step-current">
				...
				<div class="cart-order__body">
					<div class="cart-order__tip colorPrimary">От выбранного города зависят доступные виды доставки и стоимость<br/>Если мы не нашли Ваш населенный пункт, то укажите его в комментарии к заказу</div>
					...
					<div class="form-group-inline form-group--city">
						...
						?>
						<input type="hidden" name="ORDER_PROP_16" id="soa-property-16" value="<?=$arLocs['CITY_NAME']?>">
					...
					<div class="cartDeliveryAddress" style="display: none">
						...
						<div class="delivery-address__group">
							...
							<div class="form-group f-group--index">
								<input type="text" name="ORDER_PROP_14" class="form-control" id="soa-property-14" placeholder="Подъезд *">
							</div>
							<div class="form-group f-group--index">
					...
					foreach($arResult['JS_DATA']['GRID']['ROWS'] as $product){
						...
						/*$all_сities = Prymery\Regionality::getAllStores();
						$realRegion = Prymery\Regionality::getRealRegionByIP();
						$allStores = $all_сities[$realRegion['NAME']]['SHOPS'];*/
							...
							<div class="product__availability">
								<div class="shops__container">
									<div class="shops__list">
										<div class="shops__count">Найдено <?=count($allStoresNew);?> <?=endingsForm(count($allStoresNew),'магазине','магазина','магазинов');?></div>
										...
										<?foreach($allStoresNew as $shop):?>
											<div class="shops__item">
												...
												<label class="color-checkbox">
													<input type="radio" data-id="mapshoplink_<?=$shop['INFO']['ID']?>" data-coords="<?=$shop['INFO']['UF_COORDS']?>" data-title="<?=$shop['INFO']['TITLE']?>" class="color-checkox__value mapShopDetail" name="ORDER_SHOP" value="<?=$shop['INFO']['ID']?>">
											...
											<script type="text/javascript">
												...
												function initmap() {
													...
													$('.mapShopDetail').on('change', function(){
														...
														$('input[name=ORDER_PROP_13]').val($(this).data('title'));
													});
												}
		...
		<div style="display: none">
			...
			<div id="bx-soa-delivery" data-visited="false" class="bx-soa-section bx-active" <?= ($hideDelivery ? 'style="display:none"' : '') ?>>
				<div class="bx-soa-section-title-container" style="display:none;">
					<h2 class="bx-soa-section-title col-sm-9"> <span class="bx-soa-section-title-count"></span><?= $arParams['MESS_DELIVERY_BLOCK_NAME'] ?> </h2>
					<div class="col-xs-12 col-sm-3 text-right"><a href="" class="bx-soa-editstep"><?= $arParams['MESS_EDIT'] ?></a>
					</div>
				</div>
...
?>
<script src="https://freevape.ru/local/templates/freevape/assets/js/order_ajax.js?up=1"></script>
<script>
