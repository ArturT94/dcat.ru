...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		getData: function(action, actionData)
		{
			...
			$.getJSON('/local/ajax/location_id.php',{ID: $('input[name=ORDER_PROP_6]').val()},
				function (data2) {
					data.order['ORDER_PROP_6'] = data2.CODE;
					data.order['RECENT_DELIVERY_VALUE'] = data2.CODE;
		...
		refreshOrder: function(result)
		{
			...
			//custom
			var location_prop_id;
			var props_order = result.order.ORDER_PROP.properties;
			for(var i=0;i<props_order.length;i++){
				if(props_order[i]['TYPE'] == 'LOCATION'){
					location_prop_id = props_order[i]['ID'];
				}
			}
			if(location_prop_id){
				result.locations[location_prop_id].output[0] = "<input name=''>";
				result.locations[location_prop_id]['lastValue'] = $('input[name=ORDER_PROP_'+location_prop_id+']').val();
			}
		...
		{
			//custom
			//Очищаем блок с доставками 
			$('.deliveryAjaxBlock').html('');
		...
		createDeliveryItem: function(item)
		{
			//console.log(item);
			//custom
			//создаем доставки для конкретного региона
			if(item.PRICE_FORMATED == '0 руб.'){
				item.PRICE_FORMATED = 'Бесплатно';
			}
			//Меняем принцип формирования названия для доставок с профилем (оставляем только название профиля)
			var normal_name = item.NAME.split('(');
			if(normal_name[1]){
				var normal_name = normal_name[1].split(')');
			}
			var link_map = '';
			if(item.ID == 18){
				var link_map = '<span class="cart-checkbox__description"><div id="sdekmapshow"></div></span>';
			}
			$('.deliveryAjaxBlock').append('<label class="cart-checkbox blockDelivery_'+item.ID+'">'+
				'<input type="radio" class="cart-checkbox__value delivery-method delivery-pickup-val" id="ID_DELIVERY_ID_'+item.ID+'" name="DELIVERY_ID" value="'+item.ID+'">'+
				'<span class="cart-checkbox__icon"><svg class="icon"><use xlink:href="#check"></use></svg></span>'+
				'<span class="cart-checkbox__content">'+
					'<span class="cart-checkbox__title">'+normal_name[0]+'</span>'+
					'<span class="cart-checkbox__description">'+item.DESCRIPTION+'</span>'+link_map+
					'<span class="cart-checkbox__price" data-price="'+item.PRICE+'">'+item.PRICE_FORMATED+'</span>'+
				'</span>'+
			'</label>');
			//--//
		...
		createTotalUnit: function(name, value, params)
		{
			...
			if (params.total)
			{
				className += ' bx-soa-cart-total-line-total noJS';
			}

			if (params.highlighted)
			{
				className += ' bx-soa-cart-total-line-highlighted noJS';
			}
...
//custom
//Подставляем в свойство заказа LOCATION код местоположения, а не его id 
$(document).ready(function(){
	$.getJSON('/local/ajax/location_id.php',{ID: $('input[name=ORDER_PROP_6]').val()},
		function (data2) {
			$('input[name=ORDER_PROP_6]').val(data2.CODE);
			BX.Sale.OrderAjaxComponent.sendRequest('refreshOrderAjax');
		}
	);
	$('input[name=ORDER_PROP_6]').on('change', function(){
		if($(this).val()){
			if($(this).val() != 1168 && $(this).val() !=  1284){
				$('.deliveryAjaxBlock').addClass('hideShop');
			}else{
				$('.deliveryAjaxBlock').removeClass('hideShop');
			}
			...
			$.getJSON('/local/ajax/location_id.php',{ID: $('input[name=ORDER_PROP_6]').val()},
				function (data) {
					new_val = data.CODE;
					if($(el).val() != new_val){
						$('input[name=ORDER_PROP_6]').val(new_val);
						BX.Sale.OrderAjaxComponent.sendRequest('refreshOrderAjax');
