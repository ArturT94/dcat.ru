...
BasketPoolQuantity.prototype.updateQuantity = function()
{
    ...
    {
        for (var i = 1; items.children.length > i; i++)
        {
...
function updateBasketTable(basketItemId, res)
{
            ...
            for (i = 0; i < arColumns.length; i++)
            {
                switch (arColumns[i])
                {
                    ...
                    case 'PRICE':
                        ...
                        oCellPrice.innerHTML = '<div class="current_price" id="current_price_' + arItem['ID'] + '">' + Math.ceil(arItem['PRICE_FORMATED']) + '</div>' +
        ...
        for (id in res.BASKET_DATA.GRID.ROWS)
        {
            if (res.BASKET_DATA.GRID.ROWS.hasOwnProperty(id))
            {
                ...
                if (BX('current_price_' + id))
                    BX('current_price_' + id).innerHTML = Math.ceil(item.PRICE_FORMATED);

                if (BX('old_price_' + id))
                    BX('old_price_' + id).innerHTML = (item.DISCOUNT_PRICE_PERCENT > 0) ? item.FULL_PRICE_FORMATED : '';

                if (BX('sum_' + id))
                    BX('sum_' + id).innerHTML = Math.round(item.SUM_FULL_PRICE,-1);
    ...
    if (!!res.BASKET_DATA)
    {
        ...
        if (BX('allSum_FORMATED'))
            BX('allSum_FORMATED').innerHTML = res['BASKET_DATA']['allSum_FORMATED'].replace(/\s/g, '&nbsp;').replace('руб.', '');
        if (BX('allSum_FORMATED_1'))
            BX('allSum_FORMATED_1').innerHTML = res['BASKET_DATA']['allSum_FORMATED'].replace(/\s/g, '&nbsp;').replace('руб.', '');

        if (BX('PRICE_WITHOUT_DISCOUNT'))
        {
            var showPriceWithoutDiscount = res['BASKET_DATA']['PRICE_WITHOUT_DISCOUNT'];
...
function couponCreate(couponBlock, oneCoupon)
{
    ...
    $('input[name=COUPON]').val(oneCoupon.COUPON);
    $('input[name=COUPON]').removeClass('bad');
    $('input[name=COUPON]').removeClass('good');
    $('input[name=COUPON]').addClass(couponClass);
...
function updateQuantity(controlId, basketId, ratio, bUseFloatQuantity)
{
    ...
    else
    {
        ...
        var newValInt = parseInt(newVal * 10000),
...
function recalcBasketAjax(params)
{
    ...
    if (!!items && items.children.length > 0)
    {
        for (i = 1; items.children.length > i; i++)
    ...
    BX.ajax({
        ...
        onsuccess: function(result)
        {
            ...
            //перенесем инфу в правый блок
            //$("#vp-pb").html($("#result_total").html());
            //$("#result_total").html('');
...
function reloadFunction () {
    $('.delPage').on('click', function () {
        ...
        $.getJSON('/local/ajax/update_basket_page.php',
            {
                VAL: 'UPDATE',
            },
            function (data) {
                $(".basketAjax").html(data.BASKET_HTML);
				$('.ajaxPrice').html(data.PRICE);
				$('.ajaxPrice').attr('data-all',data.PRICE);
				if($("input[name=DELIVERY_ID]:checked").parent().find('.cart-checkbox__price').data('price')>0){
					var allPriceNew = data.PRICE + $("input[name=DELIVERY_ID]:checked").parent().find('.cart-checkbox__price').data('price');
					$('.ajaxPrice2').html(allPriceNew);
				}else{
					$('.ajaxPrice2').html(data.PRICE);
				}
				$('.jsCountBasket').html(data.QUANTITY);
				location.reload();
            }
        );
    });
    $('.updateStat:not(.init)').on('click', function () {
        var id = $(this).data('id');
	...
	$('.updateStat').addClass('init');
   $('.js-select-modal').on('click', function(){
		var id = $(this).data('id');
		var parent_id = $(this).closest('.modal-postcard').attr('id');
		var parent_block = $('.'+parent_id);
		$(parent_block).addClass('added');
		$(parent_block).find('.link-remove').attr('data-id',id);
		$(parent_block).find('.checkbox-value').prop('checked',true);
		var description = $('#'+parent_id).find('textarea').val();
	...
	$(".link-remove").on("click", function(){
		...
		$(this).closest(".cart-additional__item").find(".checkbox-value").prop("checked", false);
		$(this).closest(".cart-additional__item").toggleClass("added");
	});
}
$(document).ajaxComplete(reloadFunction);
