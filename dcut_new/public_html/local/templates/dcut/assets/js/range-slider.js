...
$(function () {
    $("#filter__range-bg").slider({
        min: 0,
        max: 1000000,
        values: [200000, 800000],
        range: true,
        stop: function (event, ui) {
            $("input#priceMin-bg").val($("#filter__range-bg").slider("values", 0));
            $("input#priceMax-bg").val($("#filter__range-bg").slider("values", 1));

            $('.price-range-min.value').html($("#filter__range-bg").slider("values", 0));
            $('.price-range-max.value').html($("#filter__range-bg").slider("values", 1));
        },
        slide: function (event, ui) {
            $("input#priceMin-bg").val($("#filter__range-bg").slider("values", 0));
            $("input#priceMax-bg").val($("#filter__range-bg").slider("values", 1));
    ...
    $("input#priceMin-bg").on('change', function () {
        var value1 = $("input#priceMin-bg").val();
        var value2 = $("input#priceMax-bg").val();
        if (parseInt(value1) > parseInt(value2)) {
            value1 = value2;
            $("input#priceMin-bg").val(value1);
            $('.price-range-min.value').html(value1);
        }
        $("#filter__range-bg").slider("values", 0, value1);
        $('.price-range-min.value').html(value1);
    });

    $("input#priceMax-bg").on('change', function () {
        var value1 = $("input#priceMin-bg").val();
        var value2 = $("input#priceMax-bg").val();
        // if (value2 > 20000) {
        //     value2 = 20000;
        ...
        if (parseInt(value1) > parseInt(value2)) {
            value2 = value1;
            $("input#priceMax-bg").val(value2);
            $('.price-range-max.value').html(value2);
        }
        $("#filter__range-bg").slider("values", 1, value2);
        $('.price-range-max.value').html(value2);
    });

    $('.ui-slider-handle:eq(0)').append('<span class="price-range-min value">' + $('#filter__range-bg').slider('values', 0) + '</span>');
    $('.ui-slider-handle:eq(1)').append('<span class="price-range-max value">' + $('#filter__range-bg').slider('values', 1) + '</span>');
});
