...
if($_REQUEST['START_IMPORT_DCUT'] == 'YES') {
    ...
    curl_close($ch);
    if($result == 'ok'){
        $host = 'https://s2.dcut.ru:3310/UCS/hs/PersAcc/sales/';
            ...
            foreach($arUsers as $user){
                if($arLk[$user['UF_GUID']]){
                    ...
                    if($arResult->response->Documents){
                        foreach($arResult->response->Documents->Sales as $item){
                            unset($ID_journal2);
                            $arLoadProductArray = Array(
                            ...
                            CIBlockElement::SetPropertyValuesEx($ID_journal2, false, array('GUIDORDERS' => $item->GUIDOrders));
    ...
    require_once(dirname(__DIR__).'/Repairs/RepairsSale.php');    
}