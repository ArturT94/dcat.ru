...
if($_REQUEST['START_IMPORT_DCUT'] == 'YES') {
    ...
    curl_close($ch);
    if($result == 'ok'){
        $host = 'https://s2.dcut.ru:3310/UCS/hs/PersAcc/repairs/';
            ...
            foreach($arUsers as $user){
                if($arLk[$user['UF_GUID']]){
                    ...
                    if($arResult->response->Documents){
                        foreach($arResult->response->Documents->PurchaseOrders as $item){
                            unset($ID_journal2);
                            $arLoadProductArray = Array(
                            ...
                            CIBlockElement::SetPropertyValuesEx($ID_journal2, false, array('GUIDPRODUCT' => $item->GUIDProduct));
                            ...
                            CIBlockElement::SetPropertyValuesEx($ID_journal2, false, array('TYPEOFWORK' => $item->TypeOfWork));
                            CIBlockElement::SetPropertyValuesEx($ID_journal2, false, array('PRODUCT' => $item->Product));
                            CIBlockElement::SetPropertyValuesEx($ID_journal2, false, array('PRODUCTSKU' => $item->ProductSKU));
                            CIBlockElement::SetPropertyValuesEx($ID_journal2, false, array('SHIPPED' => $item->Shipped));
                            CIBlockElement::SetPropertyValuesEx($ID_journal2, false, array('DAYSINREPAIR' => $item->DaysInRepair));
                            CIBlockElement::SetPropertyValuesEx($ID_journal2, false, array('SERIALNUMBER' => $item->SerialNumber));
  ...
  require_once(dirname(__DIR__).'/Repairs/Repairs.php');
}